<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Pengaturan Blog <a href="{{route('member.blog.create')}}" class="bg-blue-400 p-2 rounded-md text-white text-sm">Tambah Tulisan</a>
        </h2>
    </x-slot>
    <x-slot name="headerRight">
        <form action="{{ route('member.blog.index')}}" method="get">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border px-6 py-4 text-center">
                                    <form action='{{route('member.blog.destroy', ['post'=>$value->id])}}' method="POST" onsubmit="return confirm('Apakah anda yakin akan menghapus data ini?')">
                                        @csrf
                                        @method('DELETE')

                                            
                                    <a href='{{route('member.blog.edit', ['post'=>$value->id])}}' class="text-blue-600 hover:text-blue-400 px-2">edit</a>
