    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        <form method="post" action="{{route('member.user.update', $user->id)}}" class="space-y-6" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <header>
                            ...
                            <div>
                                ...
                                <x-text-input id="email" name="email" value="{{ old('email', $user->email)}}" type="text" class="mt-1 block w-full" />
                            </div>
                            <div>
                            ...
                            <div>
                                <input type="checkbox" class="border-gray-300 rounded-md"  name="permission[]" value="{{ $value->name }}" {{ (old('permission', $userPermission) && in_array($value->name, old('permission', $userPermission))) ? 'checked' : ''}}/>
