                ...
                <div class="row gx-4 gx-lg-5 justify-content-center">
                    <div class="col-md-10 col-lg-8 col-xl-7">
                        <div class="site-heading">
                            <a href="{{ $pageHeaderLink }}" class="text-white"><h1>{{ $pageHeader }}</h1></a>
                            @isset($pageSubheading)
                            <span class="subheading">{{ $pageSubheading }}</span>
                            @endisset

                            @isset($pageUser)
                            <span class="meta">Posted By {{ $pageUser }}</span> on {{ $pageDate }}</span>
                                
                            @endisset

                            @isset($content)
                            <span style="font-family: 'Roboto', sans-serif; font-size: 2rem;">{{ $content }}</span>
                            @endisset
